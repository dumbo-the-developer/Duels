...
public class DuelCountdown implements Runnable {
    ...
    private final AtomicReference<SchedulerAdapter.TaskWrapper> scheduledTask = new AtomicReference<>();
    ...
    public void run() {
        if (!arena.isUsed() || index >= messages.size()) {
            ...
            SchedulerAdapter.TaskWrapper task = scheduledTask.get();
            if (task != null) {
                task.cancel();
            }
    ...
    public void startCountdown(long delay, long period) {
        SchedulerAdapter.TaskWrapper task = DuelsPlugin.getSchedulerAdapter()
