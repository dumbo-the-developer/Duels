...
public class DuelsPlugin extends JavaPlugin implements Duels, LogSource {
    ...
    public void onEnable() {
        ...
        // Load dependencies first using LibraryLoader (supports both Spigot and Paper)
        DuelsLibraryLoader libraryLoader = new DuelsLibraryLoader(this);
    ...
    private void checkForUpdatesAndMetrics() {
        ...
        if (updateManager.updateIsAvailable()){
            sendMessage("&a===============================================");
            sendMessage("&aA newer version of " + getName() + " is available!");
            sendMessage("&aYour version: &f" + updateManager.getCurrentVersion() + " &a| Latest: &f" + updateManager.getLatestVersion());
            sendMessage("&aDownload at: &e" + getDescription().getWebsite());
            sendMessage("&a===============================================");
        }
    }
}
