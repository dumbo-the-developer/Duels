...
public class ItemsLegacy implements ItemsCompat {
    ...
    public ItemStack createPotion(Material material, PotionType type, boolean extended, boolean upgraded) {
        ...
        if (type == null) type = PotionType.INSTANT_HEAL;
        if (extended) extended = false;
        if (!upgraded) upgraded = true; // default value, you can change it if you want

        ItemStack potion = ItemBuilder.of(material)
                .potion(type, extended, upgraded)
                .build();

        // Make it a splash potion
        potion.setDurability((short) 16384);
    ...
    public boolean isHealSplash(ItemStack item) {
        ...
        return item.getDurability() == 16373 || item.getDurability() == 16374;
    }
