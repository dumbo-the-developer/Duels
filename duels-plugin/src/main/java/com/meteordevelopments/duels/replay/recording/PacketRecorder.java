...
import com.meteordevelopments.duels.replay.recording.optimization.ReplayOptimizer;
import com.meteordevelopments.duels.replay.util.NPCManager;
import com.meteordevelopments.duels.replay.util.entities.EntityMappings;
		...
		this.packetAdapter = new PacketAdapter(DuelsPlugin.getInstance(), ListenerPriority.HIGHEST, RECORDED_PACKETS) {
			...
			public void onPacketSending(PacketEvent event) {
				...
				if (event.getPacketType() == PacketType.Play.Server.SPAWN_ENTITY) {
					...
					com.meteordevelopments.duels.replay.packet.old.WrapperPlayServerSpawnEntity oldPacket = new com.meteordevelopments.duels.replay.packet.old.WrapperPlayServerSpawnEntity(event.getPacket());
				...
				if (event.getPacketType() == PacketType.Play.Server.SPAWN_ENTITY_LIVING && ConfigManager.RECORD_ENTITIES) {
					...
					if (!spawnedEntities.containsKey(packet.getEntityID())) {
						...
						if (VersionUtil.isCompatible(VersionEnum.V1_8)) {
							com.meteordevelopments.duels.replay.packet.old.WrapperPlayServerSpawnEntityLiving oldPacket = new com.meteordevelopments.duels.replay.packet.old.WrapperPlayServerSpawnEntityLiving(event.getPacket());
							location = new LocationData(oldPacket.getX(), oldPacket.getY(), oldPacket.getZ(), p.getWorld().getName());
						} else {
							location = new LocationData(packet.getX(), packet.getY(), packet.getZ(), p.getWorld().getName());
						}
