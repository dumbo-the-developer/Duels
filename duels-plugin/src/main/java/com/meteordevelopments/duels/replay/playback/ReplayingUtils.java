...
import com.meteordevelopments.duels.replay.util.legacy.LegacyBlock;
...
import com.meteordevelopments.duels.replay.recording.PlayerWatcher;
import com.meteordevelopments.duels.replay.util.MetadataBuilder;
import com.meteordevelopments.duels.replay.util.NPCManager;
import com.meteordevelopments.duels.replay.util.entities.*;
...
public class ReplayingUtils {
	...
	public void handleAction(ActionData action, ReplayData data, ReplayingMode mode) {
		...
		if (action.getType() == ActionType.PACKET && this.replayer.getNPCList().containsKey(action.getName())) {
			...
			if (action.getPacketData() instanceof ChatData) {
				...
				String formatted = ConfigManager.CHAT_FORMAT
						.replace("{name}", action.getName())
						.replace("{message}", chatData.getMessage());
				replayer.sendMessage(formatted);
			}
		...
		if (action.getType() == ActionType.DESPAWN || action.getType() == ActionType.DEATH) {
			if (!reversed && replayer.getNPCList().containsKey(action.getName())) {
				...
				if (action.getType() == ActionType.DESPAWN) {
					replayer.sendMessage("§6" + action.getName() + " §7left the game.");
				} else {
					replayer.sendMessage("§6" + action.getName() + " §7died.");
				}
	...
	private void playInvUpdate(INPC npc, InvData invData) {
		if (!VersionUtil.isCompatible(VersionEnum.V1_8)) {
			...
			List<com.meteordevelopments.duels.replay.packet.old.WrapperPlayServerEntityEquipment> equipment = NPCManager.updateEquipmentOld(npc.getId(), invData);
			PacketNPCOld oldNPC = (PacketNPCOld) npc;
			oldNPC.setLastEquipmentOld(equipment);

			for (com.meteordevelopments.duels.replay.packet.old.WrapperPlayServerEntityEquipment packet : equipment) {
				packet.sendPacket(replayer.getWatchingPlayer());
			}
		}
	}
	...
	private void spawnProjectile(ProjectileData projData, FishingData fishing, World world, int id) {
		if (projData != null && projData.getType() != EntityBridge.FISHING_BOBBER.toEntityType()) {
			...
			Location loc = LocationData.toLocation(projData.getSpawn());
			DuelsPlugin.getFoliaLib().getScheduler().runAtLocation(loc, task -> {
				Projectile proj = (Projectile) world.spawnEntity(loc, projData.getType());
	...
	public void resetChanges(Map<Location, ItemData> changes) {
		DuelsPlugin.getFoliaLib().getScheduler().runNextTick(task -> setBlocks(changes));
	}
