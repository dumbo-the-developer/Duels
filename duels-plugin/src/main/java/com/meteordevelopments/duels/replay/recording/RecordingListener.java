...
import com.meteordevelopments.duels.replay.util.ItemUtils;
...
public class RecordingListener extends AbstractListener {
	...
	public void onJoin(PlayerJoinEvent e) {
		...
		if (!this.recorder.getPlayers().contains(p.getName()) && (this.replayLeft.contains(p.getName())) || ConfigManager.ADD_PLAYERS) {
			...
			this.recorder.addData(this.recorder.getCurrentTick(), new ActionData(this.recorder.getCurrentTick(), ActionType.MESSAGE, p.getName(), new ChatData("§6" + p.getName() + " §7joined the game.")));
